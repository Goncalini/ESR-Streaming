...
class Client:
    ...
    def send_and_receive(self, s, message, ip, port, timeout = 2.0 , retries = 3):
    ...
    def send_and_receive2(self, socket, message, ip, port):
        try:
            socket.sendto(message, (ip, port))
            data, _ = socket.recvfrom(1024)
            time.sleep(1)
            if data.decode() == "timestamp":
                return data
            else:
                return "failed"
        except Exception as e:
...
#
    ...
    def get_best_point(self) -> None:
        ...
        for point in self.points_of_presence.keys():
            latency = self.points_of_presence[point]
            if latency < best_latency:
                best_latency = latency
                best_point = point

        if best_point is not None and best_point != self.current_best_pop:
...
##
    ...
    def interrupt_stream(self, point):
        ...
        socket_pop.sendto("interrupt".encode(), (point, MonitorPOP))
...
#
    ...
    def monitor_point_of_presence(self):
        for point in self.points_of_presence.keys():     
            thread = threading.Thread(target=self.start_monitoring, args=(point,))
            thread.start()
